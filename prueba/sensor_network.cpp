...
void sensorNetwork::process_input_file(istream &iss)
{
  ...
  if(int posr = s.find('\r')) //Con esto se soluciona lo del '\r'
    s.erase(posr);
    ...
    else
    {
      ...
      if(j != quantity) //Si j es distinto a la cantidad, entonces, el sensor de consulta está en la red.
      {
        if(get_sensor_data_size(j)-1 < init)
          ...
          //if(
          node aux = network[j].queryTree(0,0,this->get_sensor_data_size(j), init, end); //Si esta funcion devuleve falso es porque no hay data
          oss<<aux;  
      ...
      {                                             //promedio de todos los sensores
        ...
        {
          int lenght = this->get_sensor_data_size(0);
          sensor mega = this->merge_beta(0,lenght);
          mega.loadTree();
          /*if(*/
          node aux = mega.queryTree(0,0,lenght,0, 1);
          cout<<"hola10"<<endl;
          oss<<aux;
          //oss<<mean<<DELIM<<min<<DELIM<<max<<DELIM<<end+1-init-unused<<endl;
          /*else
